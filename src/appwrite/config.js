...
export class Service {
  ...
  async updatePost(slug, {title, content, featuredImage, status}){
    try {
      return await this.databases.updateDocumentt(conf.appwriteDatabaseId, conf.appwriteCollectionId, slug, {
        title, content, featuredImage, status
      }
    )
      
    } catch (error) {
      console.log("Appwrite services ::updateDocument() :: ", error);
      return false
    }
  }
  ...
  async deletePost(slug){
    try {
     await this.databases.deleteDocumentt(conf.appwriteDatabaseId, conf.appwriteCollectionId, slug,
    )
      return true;
    } catch (error) {
      console.log("Appwrite services :: deleteDocument() :: ", error);
      return false
    }
  }
